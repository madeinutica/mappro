...
-- Run this in Supabase SQL Editor after the firebase migration
...
BEGIN
    ...
    -- Drop all policies on photos table if it exists
    IF EXISTS (
...
-- Create new policies for Supabase auth via user_clients
...
-- Keep the public read policy for published projects
...
-- Keep the public read policy for reviews on published projects
...
-- Photo policies (if photos table exists)
DO $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM information_schema.tables
        ...
        RAISE NOTICE 'Created photos table policies for Supabase authentication';
    ELSE
        RAISE NOTICE 'Photos table does not exist, skipping photo policy creation';
    END IF;
END $$;

-- Remove the Firebase-specific function since we're not using it
DROP FUNCTION IF EXISTS get_client_id_from_firebase_uid(TEXT);